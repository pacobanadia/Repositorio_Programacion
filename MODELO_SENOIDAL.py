...
try:
    ...
    # Instrucciones de instalación en https://pytorch.org/get-started/locally/
...
class ModeloSenoidal:
    ...
    def construir_modelo(self):
        ...
        self.modelo = nn.Sequential(
            ...
            # Activación personalizada basada en la derivada de un Hamiltoniano efectivo
            HamiltonianActivation(),
    ...
    def entrenar(self, epochs: int = 2000, lr: float = 0.01):  # Entrenar el modelo
        ...
        if self.modelo is None or self.x_entrenamiento is None:  # Verificación previa
            # | Error si no se ha construido el modelo o generado datos
            raise ValueError(
                "Debe construir el modelo y generar datos primero.")

        criterion = nn.MSELoss()  # Función de pérdida: Error Cuadrático Medio
        optimizer = torch.optim.Adam(
            self.modelo.parameters(), lr=lr)  # Optimizador Adam
        ...
        for epoch in range(epochs):  # Ciclo de entrenamiento
            ...
            if (epoch + 1) % 200 == 0:  # Reportar cada 200 épocas para no saturar consola
                ...
                # Reporte de pérdida
                print(
                    f"Época [{epoch + 1}/{epochs}] | Loss Train: {loss.item():.6f} | Loss Val: {loss_val.item():.6f}")
    ...
    def graficar_resultados(self, nombre_archivo="prediccion_vs_real.png"):
        ...
        plt.plot(x_test, y_test_real, label="Real (sin(x))", color="blue",
                 alpha=0.6, linewidth=2)  # Línea más gruesa para mejor visibilidad
        plt.plot(x_test, y_test_pred, label="Red Neuronal", color="red",
                 linestyle="--", linewidth=2)  # Línea discontinua para distinguir
        ...
        plt.scatter(self.x_entrenamiento.numpy(), self.y_entrenamiento.numpy(),
                    ...
                    s=5, color="gray", alpha=0.3, label="Datos Entrenamiento")
        ...
        plt.plot(self.historial_perdidas, label="MSE Loss",
                 color="purple")  # Color más visible
        # Título descriptivo
        ...
        # Escala logarítmica para ver mejor la convergencia final
        plt.yscale("log")
    ...
    # Guardar parámetros del modelo entrenado
